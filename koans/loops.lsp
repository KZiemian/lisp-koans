...
(define-test test-loop-in-vs-loop-on
    (let* ((letters '(:a :b :c))
      ...
      (assert-equal ____ loop-result-in)
      (assert-equal ____ loop-result-on)))
...
(define-test test-loop-across-2d-array
    (let* ((my-array (make-array '(3 3) :initial-contents
				 '((0 1 2) (3 4 5) (6 7 8))))

           (loop-result
	     (loop for i from 0 below (array-total-size my-array)
...
(define-test test-loop-across-2d-array-respecting-shape
    (let* ((my-array (make-array '(3 2) :initial-contents
				 '((0 1) (2 3) (4 5))))

           (loop-result
	     (loop for i from 0 below (array-dimension my-array 0)
		   collect
		   (loop for j from 0 below (array-dimension my-array 1)
...
(define-test test-loop-over-hash-tables
    (let* ((pairs-in-table
      ...
      (true-or-false? ____ (find '("The Hobbit" "Bilbo")
				 pairs-in-table :test #'equal))))
